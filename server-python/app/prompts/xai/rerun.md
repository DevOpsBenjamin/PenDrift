...
Forbidden — `« »`, curly `“ ” ‘ ’`, paired ASCII `'` wrapping a phrase. Use `"…"` instead. The narrative prompt downstream enforces ASCII-only quoting; if the existing template contains any non-ASCII variant or paired single-quote phrase, normalize it as you produce the improved version (this counts as a fix, not a regression).
